#!/bin/bash
if [ $# -ne 4 ]; then
  echo "Usage: $0 [Executable] [Input Data] [Input Data 2] [Dir name] "
  exit;
fi
executable=$(pwd)/$1
datasets=$2
...
# Prompt MC
prmc=/afs/cern.ch/work/k/kyolee/private/CMSSW_8_0_0/src/2Dfitting/rooDataSet/outRoo_PRMC_Pbp_newcut/outRoo_PRMC_Pbp_newcut.root
# Non-prompt MC
npmc=/afs/cern.ch/work/k/kyolee/private/CMSSW_8_0_0/src/2Dfitting/rooDataSet/outRoo_NPMC_Pbp_newcut/outRoo_NPMC_Pbp_newcut.root
...
dataMerge=2 # number of input data files to be merged
mcMerge=1 # number of input mc files to be merged
isPA=1 # 0:pp, 1:Pbp, 2:pPb, 3:pAMerged
...
cterrfile=$(pwd)/outCtErr/PbpAndpPb/fit_ctauErrorRange_Pbp.txt
ctaurange=1.5-3.0
