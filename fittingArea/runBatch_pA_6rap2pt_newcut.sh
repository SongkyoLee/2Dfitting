...
eventActivity=2 #0:nothing 1:Ntrack 2:ET^{HF}
...
cterrfile=$(pwd)/outCtErr/fit_ctauErrorRange_pA_6rap2pt_newcut.txt
ctaurange=1.5-3.0
...
ptbins=(5.0-6.5 6.5-30.0)
#ethfbins=(0.0-120.0)
#ntrkbins=(0.0-350.0)
ethfbins=(0.0-20.0 20.0-30.0 30.0-120.0)
ntrkbins=(0.0-350.0)
...
for sys in ${sysString[@]}; do
  for rap in ${rapbins[@]}; do
    for pt in ${ptbins[@]}; do
      for ethf in ${ethfbins[@]}; do
        program $sys $rap $pt $ntrkbins $ethf
      done
    done
  done
done
