...
//  if (!inOpt.ptrange.compare("2.0-3.0") ){ lmin = 3.0; lmax = 5.0; } // TEST
  ...
  TFile fInPRMC(inOpt.FileNamePRMC.c_str());  //// Prompt J/psi MC
  cout << inOpt.FileNamePRMC.c_str() << endl;
  if (fInPRMC.IsZombie()) { cout << "CANNOT open prompt MC file\n"; return 1; }
  fInPRMC.cd();
  RooDataSet *dataPRMC;
  if (inOpt.doWeight == 1) {
  ...
  TFile fInNPMC(inOpt.FileNameNPMC.c_str());   //// Non-prompt J/psi MC
  ...
  TFile fInData(inOpt.FileNameData.c_str()); //// Data
  ...
  data->SetName("data");
	...
	if (inOpt.dataMerge) {
		...
		cout << "## NOTICE : 2nd data to be merged (only for Pbp) \n";
		cout << inOpt.FileNameData2.c_str() << endl;
  	if (fInData2->IsZombie()) { cout << "CANNOT open 2nd Datadata file\n"; return 1; }
  	fInData2->cd();
  	RooDataSet *data2;
		if (inOpt.doWeight == 1 ) {
			data2 = (RooDataSet*)fInData2->Get("dataJpsiWeight");
	    cout << "## WEIGHTED dataset is used\n";
	 	 } else {
...
//	if (!inOpt.ptrange.compare("2.0-3.0")){ws->var("Jpsi_Ct")->setBins(100);}  // TEST
  ...
  struct PARAM {
	...
	////// *** mSigFunct + mBkgFunct
	sprintf(funct,"SUM::MassPDF(NSig[%f,1.0,50000.0]*%s,NBkg[%f,1.0,500000.0]*%s)",initSig,initBkg,inOpt.mSigFunct.c_str(),inOpt.mBkgFunct.c_str());
			...
			if (inOpt.EventActivity == 0) { // no multiplicity
				if (inOpt.isPA == 1) {
					if (!inOpt.yrange.compare("-2.4--1.97")) {
						if (!inOpt.ptrange.compare("0.0-1.5")){
						}
					}
				} //isPA==2
			} // EventActivity==0
*/
  ...
  if(inOpt.PcombinedWidthErr < 1) inOpt.PcombinedWidthErr = 1;
	...
	//// *** Draw inclusive mass plots, if doBfit==0
...
*/
  ...
  drawFinalCtau(ws, redData, binDataCtErr, NSigNP_fin, NBkg_fin, fit2D, lmin, lmax, inOpt, &UnNormChi2_time, &nFitParam_time, &nFullBinsPull_time, &Dof_time, &Chi2_time);
	...
	//// **** Check final resoltuion function with data
	//RooRealVar* CtWeightedD = new RooRealVar("CtWeightedD","#font[12]{l}_{J/#psi} / #sigma( #font[12]{l}_{J/#psi} )",-5.,5.); ws->import(*CtWeightedD);
...
void parseInputArg(int argc, char* argv[], InputOpt &opt) {
  ...
  opt.isPA = 1;	//pA (0:pp, 1:Pbp, 2:pPb)
	opt.EventActivity = 0;
 
  opt.doBfit = 0;

	opt.readCtErr = 1; //0: ctau error range will be inserted from other file, 1: normal 2: const value
      ...
      case '-':{
        switch (tmpargv[1]) {
          case 'f':
            ...
            cout << "  ** Data file: " << opt.FileNameData << endl;
						if (opt.dataMerge) {
						...
						break;
          case 'm':
            opt.FileNamePRMC = argv[i+1];
            opt.FileNameNPMC = argv[i+2];
            cout << " * Prompt MC data file: " << opt.FileNamePRMC << endl;
            cout << " * Non-prompt data file: " << opt.FileNameNPMC << endl;
            break;
          case 'w':
            opt.doWeight = atoi(argv[i+1]);
            cout << " * (Zvtx) Weighting MC: " << opt.doWeight << endl;
            break;
          case 'c':
            opt.isPA = atoi(argv[i+1]);
            cout << " * beam info (0:pp, 1:Pbp, 2:pPb): " << opt.isPA << endl;
...
void formTitle(InputOpt &opt) {
    ...
    sprintf(opt.lumi,"L_{int} = 21 nb^{-1}");
		...
		sprintf(opt.cmspre,"CMS Preliminary");
		sprintf(opt.beamEn,"pp  #sqrt{s_{NN}} = 5.02 TeV");
		sprintf(opt.lumi,"L_{int} = 14 nb^{-1}");
	}
}
